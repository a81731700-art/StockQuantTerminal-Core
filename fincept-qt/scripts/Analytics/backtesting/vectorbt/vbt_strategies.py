...
def build_strategy_signals(
    ...
    if builder is None:
        custom = _load_custom_strategy(strategy_type)
        if custom is not None:
            return custom['build_signals'](vbt, close_series, parameters, high_series, low_series, volume_series)
        raise ValueError(
...
_CUSTOM_DIR = Path(__file__).parent / 'custom_strategies'


def _discover_custom_strategies() -> list:
    """Load drop-in StockQuant strategy modules from custom_strategies/*.py."""
    found = []
    ...
    for path in sorted(_CUSTOM_DIR.glob('*.py')):
        if path.name.startswith('_'):
            continue
        try:
            spec = importlib.util.spec_from_file_location(f'stockquant_custom_{path.stem}', path)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            meta = getattr(module, 'STRATEGY', None)
            build = getattr(module, 'build_signals', None)
            if isinstance(meta, dict) and callable(build) and meta.get('type'):
                found.append({'meta': meta, 'build_signals': build})
        except Exception as exc:
            print(f'[CUSTOM-STRATEGY] Failed {path.name}: {exc}', file=__import__('sys').stderr)
    return found


def _load_custom_strategy(strategy_type: str):
    for item in _discover_custom_strategies():
        if item['meta'].get('type') == strategy_type:
            return item
    return None
...
def get_strategy_catalog() -> list:
    ...
    catalog.extend(item['meta'] for item in _discover_custom_strategies())
    return catalogVBT Strategy Library
